...
void SeqAuthDialog::determineState()
{
    ...
    if (gota && gotb) {
        ...
        mState = trial;
        txt =
        "Please enter your Purchase ID and CODE below and click 'Authorize' If you are not online, click'Offline'";
    }
    ...
    switch (id) {
        ...
        case SEQCTL_AUTH_OFFLINE:
            ...
            mLblDescription->setText("Please visit " SEQ_LICENSE_URL_FRIENDLY
                                     ", enter your key below and click 'Authorize'", juce::dontSendNotification);
...
SeqModalDialog(glob, SEQCTL_AUTHDIALOG, parent, 260, 200) {
    ...
    mBtnAuth = addStdButton("AUTHORIZE", 0, SEQCTL_AUTH_AUTHORIZE);
    mBtnClose = addStdButton("CLOSE", 0, SEQCTL_AUTH_CLOSE);
    mBtnOffline = addStdButton("OFFLINE", 0, SEQCTL_AUTH_OFFLINE);
